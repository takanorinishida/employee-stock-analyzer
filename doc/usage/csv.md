...
```bash
stock csv export <plan_id> ./transactions.csv
...
`avg_cost_with` 以降の列は計算済み値です。インポート時には無視されます。
...
```bash
stock csv import <plan_id> ./transactions.csv
# 15 件をインポートしました。
```

### インポート用 CSV のフォーマット

インポートに必要な列は以下の 9 列です。
...
```csv
transaction_date,transaction_type,shares_quantity,contribution_amount,incentive_amount,dividend_amount,sale_price_per_share,split_ratio_before,split_ratio_after
2024-01-10,CONTRIBUTION,10,10000,500,,,, 
